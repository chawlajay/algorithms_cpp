...
int get_lcm(int a,int b)            // you know what to do
{
...
void construct_segment_tree(int t[],int a[],int v,int tl,int tr){
    ...
    else
    {
        ...
        t[v]=get_gcd(t[2*v],t[2*v+1]);
    }
}

int gcd_query(int t[],int v,int tl,int tr,int l,int r)
{
    if(l>r)
    return 0;
    if(l==tl && r==tr)
    ...
    int tm=(tl+tr)/2;
    return get_gcd(gcd_query(t,2*v,tl,tm,l,min(r,tm)),gcd_query(t,2*v+1,tm+1,tr,max(l,tm+1),r));
}
...
void solve()
{
  ...
  while(q--)
  {
      ...
      cout<<"GCD of elements from position "<<l<<" to "<<r<<" is "<<gcd_query(segmentTree,1,0,n-1,l-1,r-1)<<"\n";
      else 
