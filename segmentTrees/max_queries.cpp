...
int max_query(int t[],int v,int tl,int tr,int l,int r)
{
    if(l>r)
    return 0;
    if(l==tl && r==tr)
    ...
    int tm=(tl+tr)/2;
    return max(max_query(t,2*v,tl,tm,l,min(r,tm)),max_query(t,2*v+1,tm+1,tr,max(l,tm+1),r));
}
...
void solve()
{
  ...
  while(q--)
  {
      ...
      cout<<"Maximum of elements from position "<<l<<" to "<<r<<" is "<<max_query(segmentTree,1,0,n-1,l-1,r-1)<<"\n";
      else 
